...
async fn test_agent_runtime_isolation_and_secret_passthrough() {
    ...
    const LOCAL_AI_KEY: &str = "***";

    // 1. Push the local AI API key to the worker (desktop vault passthrough).
    let secret = Secret::new("AI_API_KEY", "llm", LOCAL_AI_KEY.as_bytes().to_vec());
    ...
    // 2. Push a local MCP server that requires the AI API key.
    ...
    // 3. Run two agents that both use the same shared MCP server.
    ...
    // 4. Collect AgentFinished events for both agents.
    ...
    // 5. Verify workspace isolation: each agent has its own directory.
    ...
    // 6. Verify workspace isolation and secret passthrough: each agent
    ...
    // 7. Verify MCP cache is shared: only one install directory for runtime-mock.
