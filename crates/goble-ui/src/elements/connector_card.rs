...
impl ConnectorCard {
    pub fn new(
        ...
        let root = Self::build_root(icon, title, description, tags, action, app);
        Self {
            root,
    ...
    fn build_root(
        ...
        if let Some(action) = action {
            header = header.with_child(action);
        }
...
impl Element for ConnectorCard {
    fn layout(
        &mut self,
...
mod tests {
    ...
    #[test]
    fn connector_card_click_fires_callback() {
        let clicked = Rc::new(RefCell::new(false));
        let clicked_clone = clicked.clone();
        let app = AppContext::default();
        let mut card = ConnectorCard::new(
            Icon::new("plug").finish(),
            "GitHub",
            "Read and write repository issues and pull requests.",
            ["issue", "pr"],
            None,
            &app,
        )
