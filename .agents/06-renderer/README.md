...
- [`agent-parser.md`](agent-parser.md) — the agent's output turned into UI: the Markdown → fragment → block pipeline, tool-call presentation and status, and what streaming does to both.
- [`agent-terminal-bridge.md`](agent-terminal-bridge.md) — the agent + terminal seam: how a command the agent runs becomes a real block in the pane's list and its output becomes the tool result, by claiming the next shell hook rather than by framing the stream.
