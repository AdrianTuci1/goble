...
- **`crates/goble-terminal` is the seam** that keeps Alacritty's types out of the rest of the codebase. `keys.rs` — host→PTY key encoding with the xterm modifier form. `hooks.rs` — the shell-hook channel: a byte tap that lifts a `DCS $d <hex(JSON)> ST` envelope out of the PTY stream, decodes it, and passes every other byte through untouched, including foreign `DCS`. `mouse.rs` — host→PTY pointer reports, gated by the negotiated mode (1000/1002/1003, SGR 1006, middle button and modifiers). `osc.rs` — an observer for the OSC numbers the parser discards (`7` cwd, `133` prompt markers), which passes every byte through. `palette.rs` — resolving a cell's colour to RGB, over the `xterm` sixteen plus the 6×6×6 cube and the grey ramp. `screen.rs` — one terminal surface (grid, cursor, modes, alt screen, query formatters) plus a plain row/cell snapshot for the renderer. `blocks.rs` — the command-block model of [`terminal-blocks.md`](terminal-blocks.md). 106 unit tests plus a 27-fixture conformance suite, `cargo check`/`clippy` clean, no app binary built.
...
- **The pane is switched over.** `app/src/emulator.rs` owns one screen plus both byte taps behind a lock the reader thread feeds and the UI thread samples; `app/src/ui/terminal.rs` measures the pane with the font's cell pitch, tells the session the grid size, and paints the rows through `goble_ui::elements::TerminalGrid` — one glyph per cell with per-cell colour, attributes and cursor, not a line buffer. Keys and pointer events go through `goble_terminal`'s encoders, so a program in a pane sees the modes it negotiated.
- **Status is tracked in [`.agents/TRACKER.md`](../TRACKER.md)**, not here: the phases below describe the emulator's capability set.
...
**Correction (phase 6, verified): the upstream fixture corpus is *not* fetchable.** `scripts/fetch-alacritty-terminal.sh <dir>` extracts the published `alacritty_terminal-0.26.0.crate`, and that archive contains **only `src/`** — no `tests/`, no fixtures (the script's own deletion of `tests/` is a no-op against this archive). Upstream's recordings therefore cannot be used as a differential oracle. A corpus kept outside the repository is supplied instead through `GOBLE_TERMINAL_CONFORMANCE_DIR` (a `;`/`:`-separated list of fixture directories); the checked-in corpus is the one under `crates/goble-terminal/tests/conformance/`.
...
**Fixed:** the PTY was hard-coded to 24×96 with no caller for `TerminalSession::set_size`. The pane now measures itself from the font's cell pitch every layout frame, resizes the screen and the pty together when the cell count changes, and reports the cell metrics so the pixel-sized replies are right (`TerminalSession::set_size`).
...
**Test infrastructure.** Their CI entry point is `script/presubmit`: format, an inline-test-module check (`script/check_no_inline_test_modules` — inline `mod tests {}` is banned), clippy with `-D warnings`, clang-format, wgslfmt, powershell lint, `cargo nextest run --no-fail-fast --workspace`, doc tests. Notably, their `.github/workflows/` contains no workflow that runs it — the suite is a local contract, not a gate. Ours is a gate now: `.config/nextest.toml` carries the profile (`fail-fast = false`, a `slow-timeout`) and `.github/workflows/ci.yml` runs `cargo nextest run -p goble-terminal --profile ci` on pushes and pull requests that touch the terminal crate, restoring `vendor/alacritty_terminal` first. `release.yml` remains manual `workflow_dispatch` only.
...
- [x] **Phase 0:** complete host→PTY key encoding — arrows, Home/End, PageUp/PageDown, Insert/Delete, F1–F20, Alt/Meta, Shift+Tab, xterm modifier form; asserted with encode-direction tables. The pane encodes every key through it (`classify_key`), and `goble-ui`'s key mapping now delivers Home/End/PageUp/PageDown/Insert/F1–F35 instead of dropping them.
Status of this list after the vendor + crate landing and the pane switch-over (§0): an item marked `[x]` exists in the crate and is covered by its tests, and — where it has a pane-side half — the pane uses it. The exit criteria in §1's table are behaviour in the pane, and the pane now paints the grid, so a phase is `[x]` when its mechanism is asserted and the pane consumes it.

- [x] Wire `TerminalSession::set_size` and stop hard-coding the PTY at 24×96 — the pane calls it with the cell count it measured and the font's cell metrics; the session resizes the screen and the pty together, and records the metrics so the text-area reply is in pixels.
...
- [x] Replace `TerminalBuffer` with a real grid: fg/bg/flags, wide-char spacers, combining extras, row storage — `Screen` over the vendored grid, painted by `TerminalGrid` (`crates/goble-ui/src/elements/terminal_grid.rs`). `TerminalBuffer` is gone; the only survivor of that shape is `TerminalSnapshot`, the plain text the chat transcript's inline block shows.
...
- [x] Implement the DEC private mode table and a correct DECRQM — asserted for every mode we implement (1, 6, 7, 12, 25, 1000, 1002, 1003, 1004, 1005, 1006, 1007, 1042, 1049, 2004, 2026) and for an unknown mode answering `0`; the answer comes from the emulator, which was already correct.
...
- [x] Add bracketed paste, focus events, mouse reporting (1000/1002/1003 + SGR 1006 with modifiers), OSC 8, 7, 133, 52 — `KeyEncoder::paste` wraps only when 2004 is on and strips escape bytes so a pasted `ESC [ 201 ~` cannot close the bracket; `KeyEncoder::focus` emits `ESC [ I`/`ESC [ O` only under 1004; `mouse.rs` encodes press/release/drag/move/wheel with modifiers and the middle button; `osc.rs` observes OSC 7 and OSC 133, which the parser discards. The pane drives all of them: focus on window focus, wheel and click reports to the program when it asked for them, and the pane's own scrollback when it did not.
- [x] Add the fixture-based conformance harness — `crates/goble-terminal/tests/conformance.rs` walks a 27-fixture corpus under `crates/goble-terminal/tests/conformance/` (4-file format: `recording.bin`/`size.json`/`config.json`/`grid.json`, enumerated from disk, `grid.json` denies unknown keys) and asserts rows, the full cursor (line, column, visibility, shape, deferred-wrap flag) and exact cells, with scrollback/alt-screen pinned by `history_size`/`alt_screen`/`content_rows`. `cargo nextest` is adopted (`.config/nextest.toml`, `.github/workflows/ci.yml`). Upstream's corpus is **not** fetchable (the published `.crate` holds only `src/`); an outside corpus is supplied via `GOBLE_TERMINAL_CONFORMANCE_DIR`.
