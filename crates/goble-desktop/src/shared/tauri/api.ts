...
export async function createAgent(
  ...
  return invoke('create_agent', { req: { name, prompt, description, tools: tools ?? [] } });
}
...
export async function createTeam(id: string, name: string, metadata: string, agentIds: string[]): Promise<TeamInfo> {
...
export async function runAgent(workerId: string, chatId: string, agentId: string, input: string): Promise<void> {
...
export async function setChatModel(chatId: string, provider: string, model: string): Promise<void> {
...
export async function installMcpServer(
  ...
  if (secretIds.length > 0) req.secret_ids = secretIds;
  return invoke('install_mcp_server', { req });
}

export async function updateMcpServer(id: string, name?: string, sourceValue?: string, secretIds: string[] = []): Promise<string> {
...
export async function updateMcpServerMeta(id: string, secretIds: string[], enabledTools: string[]): Promise<string> {
...
export async function testCallMcpTool(id: string, toolName: string, args?: Record<string, unknown>): Promise<unknown> {
