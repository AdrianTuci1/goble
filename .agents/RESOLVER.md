...
| --- | --- | --- |
| 2026-08-27 | Add SSH proxy mode to `goblin-worker` (`02-first-run-and-routing/remote-bootstrap.md`): `goblin --ssh-proxy` runs the worker on stdin/stdout reading `DesktopMessage` NDJSON lines and writing `WorkerMessage` NDJSON lines, so a remote worker can be spawned over SSH without exposing a TCP port. Foundation for warp-new-style VPS connection. | `cargo check --workspace` passed; `cargo test -p goblin-worker --lib` passed (32); manual smoke test: `echo '{"kind":"ping"}' | ./target/debug/goblin --ssh-proxy ...` produced `Pong` + `StatusReport` |
| 2026-08-27 | Map workspace default target to the router (`05-execution-router-and-targets/runtime-targets.md`): `GobleConfig` gains `workspace.default_target` (`Local` / `Remote { worker_id }`), `resolve_worker_for_target("any")` consults it, and `set_workspace_default_target` persists it to `config.toml`. Per-conversation override remains via `chats.workspace_routing`. | `cargo check --workspace` passed; `cargo test -p goble-core --lib config` passed (2); `cargo test -p goble-desktop-service` passed (28, incl. 4 new router tests) |
